...
module.exports = function(server, databaseObj, helper, packageObj) {
    ...
    var addRemoteMethod = function(app, modelName) {
            ...
            if (getAuthorisedRoles) {
                getAuthorisedRoles(server, function(err, roleList) {
                    if (err) {
                        ...
                        getSnaphyACL(modelName, roleList).then(function(snaphyACL) {
                            ...
                            .then(function(schema){
                                if(schema.header){
                                    if(schema.header.length){
                                        schema.header = sortTableHeading(schema.header, schema.tables);
                                    }
                                }
                                return schema;
                            })
        ...
        modelObj.remoteMethod('getSchema', {
            returns: {
                arg: 'schema',
                type: 'object'
            },
        ...
        modelObj.remoteMethod('getAbsoluteSchema', {
            returns: {
                arg: 'schema',
                type: 'object'
            },
    ...
    /**
     * Sort Table heading by Priority..Will sort by Ascending order
     * Lowest number top position...
     * @param headingList
     * @param tables
     */
    var sortTableHeading = function(headingList, tables){
        headingList = _.sortBy(headingList, [
            function(columnName) {
                var key = columnName.replace(".", "_");
                if(tables){
                    if (tables[key]) {
                        if (tables[key].position != undefined) {
                            return parseInt(tables[key].position);
                        }
                    }
                }

                //Return a default number then..
                return 999;
            }
        ]);
        return headingList;
               ...
               * }
                    ...
                    if (!checkRelationEditAccess_) {
                        if (relations[relationName].templateOptions.box) {
                            ...
                            schema.container[relations[relationName].templateOptions.box].schema.push(nestedSchema);
                        } else {
                            //Now add this to the schema..
                            schema.container.default = schema.container.default || initializeContainer();
                            schema.container.default.schema.push(nestedSchema);
                ...
                if ((relationObj.type === 'hasOne' || relationObj.type === 'belongsTo') && relationObj.templateOptions !== undefined) {
                    ...
                    belongsToSchema.templateOptions.foreignKey = relationObj.foreignKey === ""
                        ? relationName + 'Id'
                        : relationObj.foreignKey;
